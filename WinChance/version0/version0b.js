window.onload = function(){
	...
	for (var i = 0; i < position.length; i++) {
		while (position[i]>0){
			...
			var y=(i==1 || i==2)?position[i]: (i==0)?f1+position[i]:f2+position[i];//teamstructure
			var z = (i==1 || i==2)?position[i]:3+position[i];//slotposition
...
function showResult(){
	for (let round=1 ; round<= 15; round++){
		...
		while(n>0){
			if (maximum(team[0]) >= maximum(team[1])){
				atkteam = 0;
			}
			else {
				atkteam =1;
